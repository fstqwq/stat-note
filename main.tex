...
\titleformat{\section}
{\bfseries}			% The style of the section title
{\thesection.}				% a prefix
...
$\nabla L(\boldsymbol{\beta})=\sum_{i=1}^n\left[y_i \boldsymbol{x}_i-p_i \boldsymbol{x}_i\right]=X^T(Y-p)$
$H(\boldsymbol{\beta})=-\sum_{i=1}^n p_i\left(1-p_i\right) \boldsymbol{x}_i \boldsymbol{x}_i^T=-X^T W X$.
$P$ 是 $n \times 1$ 的概率向量，其元素为
$p_i=e^{\boldsymbol{x}_i^T \boldsymbol{\beta}} /\left(1+e^{\boldsymbol{x}_i^T \boldsymbol{\beta}}\right)$ 是 $P(y_i=1)$.
$W$ 是对角阵，对角线元素为 $p_i\left(1-p_i\right)$.迭代公式：
$
\boldsymbol{\beta}^{(j+1)}=\boldsymbol{\beta}^{(j)}-\left(X^T W X\right)^{-1} X^T(Y-p)$; \\
线性回归：最大似然函数:$\sigma$的分布函数(把$\sigma$换成$\boldsymbol{Y}-\boldsymbol{X}^{\top} \boldsymbol{\beta}$).$\sigma$服从椭球分布时，$\frac{\partial R S S}{\partial \boldsymbol{\beta}}=-2 X^T(Y-X \boldsymbol{\beta})$, 显式解 $\hat{\beta}=\left(X^T X\right)^{-1} X^T Y$. \\
{\bfseries 例} 给定 $(y_i, x_i)$, 判断 $\beta$ MLE 是否有显式解, 如无给出牛迭更新式, 写成加权最小二乘.
(1) $Y=X\boldsymbol{\beta} + \boldsymbol{\epsilon}, f(\boldsymbol{\epsilon})\propto g(\boldsymbol{\epsilon}^T\boldsymbol{\epsilon})$.
(2) $\mathrm{logit}(E(y_i|x_i)) = \boldsymbol{x}_i^T\boldsymbol{\beta}.$
(1) $\mathcal{L}(X_i, Y_i, \beta) = f(Y-X\boldsymbol{\beta}) \propto g((Y-X\boldsymbol{\beta})^T(Y-X\boldsymbol{\beta}))$,
$\hat \beta = \mathrm{argmax}_{\boldsymbol{\beta}} g((Y-X\boldsymbol{\beta})^T(Y-X\boldsymbol{\beta}))$,
$\frac {\partial \ell} {\partial \beta} = g'((Y - X \boldsymbol{\beta})^T(Y - X \boldsymbol{\beta}))\cdot 2(X^TX\boldsymbol{\beta}-X^TY)$,
$\hat \beta = (X^T X)^{-1}X^TY$.\\
(2) 
$E(y_i|\boldsymbol{x_i}) = P(y=1 | \boldsymbol{x}) = p_i$, $y_i$ 是服从 $p_i$ 的 Bernoulli 分布.
$L(\boldsymbol{X}, \boldsymbol{Y}, \boldsymbol{\beta}) = \prod [p_i]^{y_i} [1 - p_i]^{1 - y_i}$,
$\ell = \sum y_i \log p_i + (1 - y_i) \log (1 - p_i)
= \sum (y_i \log \frac{p_i}{1 - p_i} + \log (1 - p_i))$,
由于 $p_i = E(y_i | \boldsymbol{x_i})$, $\mathrm{logit}(E(y_i | \boldsymbol{x_i})) = \log \frac{p_i}{1 - p_i} = \boldsymbol{x_i}^T \boldsymbol{\beta}$
原式 $=\sum (y_i \boldsymbol{x_i}^T \boldsymbol{\beta} - \log (1 + e^{\boldsymbol{x_i}^T \boldsymbol{\beta}}))$,
$\partial \ell / \partial \boldsymbol{\beta} = \sum (y_i - \frac{e^{\boldsymbol{x_i}^T \boldsymbol{\beta}}}{1 + e^{\boldsymbol{x_i}^T \boldsymbol{\beta}}}) \boldsymbol{x_i} = \sum (y_i - p_i) \boldsymbol{x_i}$,
没有解析解, 牛顿迭代
$\boldsymbol{\beta}^{(t+1)} = \boldsymbol{\beta}^{(t)} - \left( \frac{\partial^2 \ell}{\partial \boldsymbol{\beta}^2} \right)^{-1} \frac{\partial \ell}{\partial \boldsymbol{\beta}}
= \boldsymbol{\beta}^{(t)} + [\sum \frac{\exp(\boldsymbol{x_i}^T \boldsymbol{\beta}^{(t)})}{(1 + \exp(\boldsymbol{x_i}^T \boldsymbol{\beta}^{(t)}))^2} \boldsymbol{x_i} \boldsymbol{x_i}^T]^{-1} [\sum (y_i - p_i) \boldsymbol{x_i}]$.\\
写成加权最小二乘:
令 $s=0$ 并循环迭代: 1. 令 $\boldsymbol{Z}=(Z_1, Z_2, \dots, Z_n)^T$, 这里 $Z_i = \log \left( \frac{p_i^{(s)}}{1 - p_i^{(s)}}\right) + \frac{y_i - p_i^{(s)}}{p_i^{(s)} (1 - p_i^{(s)})}$;
2. 令 $W = \mathrm{diag}(w_i^{(s)})$, 这里 $w_i^{(s)} = p_i^{(s)} (1 - p_i^{(s)})$; 3.$\hat \beta^{(s + 1)} = (X^T W X)^{-1} X^T W Z$.
\end{scriptsize}
\end{multicols}
